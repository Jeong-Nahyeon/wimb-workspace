...
<%
	...
	ArrayList<Product> saleList = (ArrayList<Product>)(request.getAttribute("saleList"));
	...
	<div class="outer">
		<div class="product-list-area">
				...
				$(function(){
					...
					$("select[name=selectOrder]").change(function(){
						...
						if($option == "new"){ // 신상품순일 경우
							location.href="<%= contextPath %>/onSaleList.pr?cpage=1";
						} else { // 그 외일 경우
							location.href="<%= contextPath %>/onSaleList.pr?cpage=1&selectOrder=" + $option;
						}
			...
			<div id="paging-bar">
                <% if(currentPage != 1) { %>
						<% if(selectOrder != null) { %>
							<a class ="btn btn-sm" href="<%= contextPath %>/onSaleList.pr?cpage=<%= currentPage - 1 %>&selectOrder=<%= selectOrder %>">&lt;</a>
						<% } else { %>
							<a class ="btn btn-sm" href="<%= contextPath %>/onSaleList.pr?cpage=<%= currentPage - 1 %>">&lt;</a>
						<% } %>	
				<% } %>
                ...
                <% for(int p=startPage; p<=endPage; p++ ) { %>
                    ...
                    <% } else { %>
						<% if(selectOrder != null) { %>
							<a class ="btn btn-sm" href="<%= contextPath %>/onSaleList.pr?cpage=<%= p %>&selectOrder=<%= selectOrder %>"><%= p %></a>
						<% } else { %>
							<a class ="btn btn-sm" href="<%= contextPath %>/onSaleList.pr?cpage=<%= p %>"><%= p %></a>
						<% } %>	
                    <% } %>
                <% } %>
