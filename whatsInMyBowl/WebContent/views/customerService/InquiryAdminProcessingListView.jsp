    ...
    <div class="outer">
        ...
        <div id="sort_box" align="right">
            ...
            <select name="" id="" onchange="window.open(value,'_self');">
                <option value="<%= contextPath %>/adminProcessingListView.inq?cpage=1" selected>처리중</option>
                <option value="<%= contextPath %>/adminCompletionListView.inq?cpage=1">처리완료</option>
            </select>
        </div>
        ...
        <table class="progressing_table">
	        ...
	        <div class="paging-area" align="center">
	            <% if(currentPage != 1) { %>
	            	<button onclick="location.href='<%= contextPath %>/adminProcessingListView.inq?cpage=<%=currentPage-1%>';">&lt;</button>
	            <% } %>
	            ...
	            <% for(int p=startPage; p<=endPage; p++) { %>
	            	...
	            	<% } else { %>
	            		<button onclick="location.href='<%= contextPath %>/adminProcessingListView.inq?cpage=<%= p %>';"><%= p %></button>
	            	<% } %>
	            <% } %>
